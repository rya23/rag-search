...
import { MultiQuerySteps } from "@/components/traces/MultiQuerySteps";
    ...
    return (
        <div className="container mx-auto p-6 max-w-6xl">
            <div className="mb-6">
                ...
                <p className="text-muted-foreground font-mono text-sm">{traceId}</p>
            </div>

            {error && (
                <Alert variant="destructive" className="mb-6">
                    <AlertDescription>Failed to load trace: {error instanceof Error ? error.message : "Unknown error"}</AlertDescription>
                </Alert>
            )}
            ...
            {trace && (
                <div className="space-y-6">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div>
                                    <div className="text-sm text-muted-foreground mb-1">Documents (k)</div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {trace.retriever_ms !== undefined && (
                                    <div>
                                        <div className="text-sm text-muted-foreground mb-1">Retrieval Time</div>
                                        <div className="font-mono text-lg">{trace.retriever_ms}ms</div>
                                    </div>
                                )}
                                {trace.generator_ms !== undefined && (
                                    <div>
                                        <div className="text-sm text-muted-foreground mb-1">Generation Time</div>
                                        <div className="font-mono text-lg">{trace.generator_ms}ms</div>
                                    </div>
                                )}
                                            ...
                                            <Card key={index} className="bg-muted">
                                                <CardHeader>
                                                    <div className="flex items-center justify-between">
                                                        ...
                                                        {doc.score !== undefined && <Badge variant="secondary">Score: {doc.score.toFixed(4)}</Badge>}
                                                    </div>
                                                </CardHeader>
                                                <CardContent>
                    ...
                    {/* Multi-Query Steps - Only show if available */}
                    {trace.multiquery_steps && (
                        <MultiQuerySteps steps={trace.multiquery_steps} />
                    )}
                </div>
            )}
        </div>
