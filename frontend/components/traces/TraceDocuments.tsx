...
function mapDocsToQueries(trace: TraceDetail) {
  ...
  const docToQueryMap = new Map<number, { queryIndex: number; query: string }>();
  
  // Build a map of document positions to their source queries
  trace.multiquery_steps.per_query_docs.forEach((queryDocs) => {
    queryDocs.docs.forEach((doc) => {
      docToQueryMap.set(doc.position, {
        ...
        <div className="space-y-3 pr-4">
          {trace.docs.map((doc, index) => (
            <DocumentCard 
              ...
              doc={doc} 
              index={index}
              queryInfo={docToQueryMap?.get(index)}
            />
          ))}
        </div>
      </ScrollArea>
...
function DocumentCard({ 
  ...
  doc: any; 
  index: number;
  queryInfo?: { queryIndex: number; query: string };
}) {
            ...
            >
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {queryInfo && (
                  <Badge 
                    variant="outline" 
                    className={`text-xs font-medium ${queryColors[queryInfo.queryIndex % queryColors.length]}`}
                  >
                    Query {queryInfo.queryIndex + 1}
                  </Badge>
                )}
              </div>
              {isOpen ? (
            ...
            {queryInfo && (
              <>
                <div>
                  ...
                  <div className={`rounded-lg border p-3 ${queryColors[queryInfo.queryIndex % queryColors.length]}`}>
